...
for frameNum = 1:numFrames
    ...
    CC{frameNum} = bwconncomp(Im(:,:,frameNum));    % CC记录连通数目和各像素点位置
    L{frameNum} = labelmatrix(CC{frameNum});    % 给各个连通在原图尺寸中做标记
    mito{frameNum} = regionprops(CC{frameNum},Im(:,:,frameNum),'Area','Centroid','Solidity','MaxFeretProperties','Extrema','MeanIntensity','WeightedCentroid','MajorAxisLength','MinorAxisLength','Perimeter','PixelIdxList'); % 获取各连通属性
    numMito(frameNum) = length(mito{frameNum}); %获取连通数
    ...
    NN{frameNum} = getNN(mito{frameNum}); % 根据质心获取两个连通间距离
    NNExtrema{frameNum} = getNNExtrema(mito{frameNum}); % 根据连通的顶点获取最短距离
    
    %Correct the angles of the mitochondria and assign NN, frame, and
    %initiliaze other parameters.
    for mitoNum = 1:numMito(frameNum)
        mito{frameNum}(mitoNum).MaxFeretAngle = angle0to180(180-mito{frameNum}(mitoNum).MaxFeretAngle); % 边界两点最长距离的直线相对于水平方向的角度
